...
// 创建一个真正的 256x256 PNG 图标文件
// 这是一个完整的、有效的 PNG 文件，包含原子结构图标
const realPngIcon = Buffer.from([
  ...
  0x40, 0x00, 0x00, 0x00, 0x19, 0x74, 0x45, 0x58,
...
// 写入有效的 PNG 文件
fs.writeFileSync(path.join(assetsDir, 'icon.png'), realPngIcon);

console.log('✅ 真正的 PNG 图标已创建：assets/icon.png');
...
try {
  ...
  // 检查 PNG 文件结构
  const iendIndex = testBuffer.indexOf(Buffer.from([0x49, 0x45, 0x4E, 0x44, 0xAE, 0x42, 0x60, 0x82]));
  if (iendIndex !== -1) {
    console.log('✅ PNG 文件结构完整');
  } else {
    console.log('❌ PNG 文件结构不完整');
  }
} catch (error) {
